    ...
    <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-5xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Full writing tests with Task 1 & Task 2</p>
            </div>
          </div>
        </motion.div>
        ...
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
          <span className="text-xs font-semibold px-2 py-1 rounded-lg" style={{ background: 'rgba(236,72,153,0.1)', color: '#ec4899' }}>Full Tests</span>
          Writing Tests
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {writingTasks.map((t, i) => (
            <motion.div key={t.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.05 }}>
              <Link href={`/writing/practice/${t.id}`} className="block glass-card p-5 group h-full">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs font-semibold px-2 py-1 rounded-lg" style={{ background: 'rgba(99,102,241,0.1)', color: '#6366f1' }}>
                    {t.htmlUrl ? 'Task 1 & 2' : t.type}
                  </span>
                </div>
